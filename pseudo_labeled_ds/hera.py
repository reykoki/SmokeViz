...
def run_model(idx):
    ...
    def get_best_file(dataloader, model, BCE_loss):
        ...
        bad_fn = '/scratch1/RDARCH/rda-ghpcs/Rey.Koki/more_data/low_iou/{}'.format(fn)
...
def doesnt_already_exists(yr, fn_heads, idx, density):
    final_dest = '/scratch1/RDARCH/rda-ghpcs/Rey.Koki/more_data/'
...
def iter_smoke(date):
    ...
    print(dt)
    print('------')
    smoke_dir = '/scratch1/RDARCH/rda-ghpcs/Rey.Koki/more_data/smoke/'
    smoke = get_smoke(dt, smoke_dir)

    if smoke is not None:
        smoke_rows = create_smoke_rows(smoke)
        print(smoke_rows)
        ray_dir = "/scratch1/RDARCH/rda-ghpcs/Rey.Koki/tmp/{}{}".format(yr,dn)
...
def main(start_dn, end_dn, yr):
    ...
    for dn in dns:
        ...
        dates.append([dn, yr])
    for date in dates:
        dn_dir = '/scratch1/RDARCH/rda-ghpcs/Rey.Koki/more_data/temp_data/{}{}/'.format(date[1], date[0])
