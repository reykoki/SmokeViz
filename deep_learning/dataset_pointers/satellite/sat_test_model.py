...
import os
import sys
sys.path.insert(1, '/projects/mecr8410/SmokeViz_code/deep_learning/')
import numpy as np
import torch
...
with open('/projects/mecr8410/SmokeViz_code/deep_learning/configs/exp{}.json'.format(exp_num)) as fn:
...
if use_ckpt == True and test_mode==False:
...
if test_mode:
    ...
    chkpt_pth = '/projects/mecr8410/SmokeViz_code/deep_learning/models/DLV3P_exp1_1719683871.pth'
    print(chkpt_pth)
    checkpoint=torch.load(chkpt_pth, map_location=torch.device(device))
    model.load_state_dict(checkpoint['model_state_dict'])
