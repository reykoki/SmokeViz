...
e_d_fns = glob.glob('*_*_exp*.py')
for fn in e_d_fns:
    e = fn.split('_')[1]
    d = fn.split('_')[0]
    exp_num = fn.split('exp')[-1].split('.py')[0]
...
for idx, decoder in enumerate(e_d_exp['decoder']):
    ...
    print(f'sbatch --export=CONFIG_FN=./configs/{decoder}_{encoder}_exp{exp_num}.py --job-name={job_name} --output=logs/{job_name}.log {run_script}')
