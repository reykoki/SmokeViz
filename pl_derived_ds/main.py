...
def is_bad_img(yr, density, dn, sat_num, start_scan, idx):
    file_list = glob.glob('{}bad_img/{}/{}/{}/{}_{}_*_{}.tif'.format(full_data_dir, yr, density, dn, sat_num, start_scan, idx))
    if file_list:
        return True
    return False
...
def sample_doesnt_exists(yr, dn, fn_head, idx, density):
    ...
    if len(file_list) > 0:
        ...
        return False
    if is_bad_img(yr, density, dn, sat_num, start_scan, idx):
        return False
    return True
